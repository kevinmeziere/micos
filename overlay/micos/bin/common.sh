...
{
  val="dvd"
  if [ -f /micos/liveusb ]; then
    val="usb"
    return
  fi
  if [ -f /micos/smartdoge ]; then
    val="smartdoge"
    return
  fi
}
...
micosImportImg()
{
  ...
  imgadm install -m $MICOS_EXTRA/micos/datasets/${name}.dsmanifest -f $MICOS_EXTRA/micos/datasets/${name}.zfs.gz
  echo "Done"
}
