...
rm -rf /opt/micos; mkdir -p /opt/micos
cd /opt/micos
...
# step 1: copy all micos files
badev=$(lofiadm -a ./tmp/boot_archive)
rm -rf ./tmp/bamnt; mkdir -p ./tmp/bamnt
mount $badev ./tmp/bamnt
rsync -avz ./tmp/bamnt/micos/* ./
umount ./tmp/bamnt
lofiadm -d $badev

# step 2: copy all resources into right places
rsync -avz $MEDIAPATH/micos ./mnt/micos-extra/

# step 3: mark this as smartdoge
touch ./smartdoge

# step 4: link /opt/micos -> /micos and make it persistent
ln -nsf /opt/micos /micos
MICOS_CUSTOM_SMF="<?xml version='1.0'?>
...
<service_bundle type='manifest' name='export'>
  <service name='site/smartdoge' type='service' version='0'>
    ...
    <method_context/>
    <exec_method name='start' type='method' exec='ln -nsf /opt/micos /micos' timeout_seconds='60'/>
...
echo $MICOS_CUSTOM_SMF >/opt/custom/smf/smartmicos.xml
